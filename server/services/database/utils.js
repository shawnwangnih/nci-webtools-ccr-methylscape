...
async function createRecordIterator(sourcePath, columns) {
    ...
    switch (fileExtension) {
        ...
        case "txt":
            return await createCsvRecordIterator(inputStream, columns, { delimiter: "\t" });
        case "xlsx":
...
async function createExcelRecordIterator(inputStream, columns) {
    ...
    const [headers] = XLSX.utils.sheet_to_json(sheet, { header: 1, range: 'A1:ZZZ1' });
    const errors = [];
    ...
    return records.map(record => recordParser(
        mapValues(record, castValue)
    ));
}
