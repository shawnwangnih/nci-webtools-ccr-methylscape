...
export const plotState = selector({
  ...
  get: ({ get }) => {
      ...
      seg.reduce((prev, curr) => {
        ...
        if (chr != 24) {
          if (prev[chr]) {
            return {
              ...prev,
              [chr]: Math.max(prev[chr], curr['loc.start']),
            };
          } else {
            return {
              ...prev,
    ...
    const segs = seg.map((e) => {
      ...
      return {
        chr: chr,
        posStart: e['loc.start'],
        posEnd: e['loc.end'],
        medianLog2Ratio: parseFloat(e['seg.median']),
        CNV: cnv(parseFloat(e['seg.median'])),
        x1: binPosOffset[chr] + e['loc.start'],
        x2: binPosOffset[chr] + e['loc.end'],
        width: e['loc.end'] - e['loc.start'],
        posStartNew: segPosOffset[chr] + e['loc.start'],
        posEndNew: segPosOffset[chr] + e['loc.end'],
      };
    });
    ...
    const useWebGl = bins.length > 1000;
    ...
    const dataTraces = dataGroupedByChr
      ...
      .map(([name, data]) => ({
        ...
        mode: 'markers',
        hovertemplate:
          'Log<sub>2</sub> Ratio: %{customdata.ratio}<extra></extra>',
        marker: {
          // autocolorscale: true,
        },
