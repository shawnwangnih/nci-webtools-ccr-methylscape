...
aws s3 cp $SOURCE/Sample_sheet_master.xlsx $DESTINATION/Sample_sheet_master.xlsx --profile $AWS_PROFILE
aws s3 cp $SOURCE/CNV/bins/ $DESTINATION/CNV/bins/ --recursive --profile $AWS_PROFILE 
aws s3 cp $SOURCE/CNV/segments/ $DESTINATION/CNV/segments/ --recursive --profile $AWS_PROFILE
